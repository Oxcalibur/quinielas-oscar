...
# =====================================================================
def is_test_file(filepath: str, content: str = None, project_configs: dict[str, str] = None) -> bool:
    ...
    if project_configs:
        config_text = "\n".join(project_configs.values())
        tp_match = re.search(r"testpaths\s*=\s*(.+)", config_text)
        if tp_match:
            testpaths = [p.strip() for p in tp_match.group(1).split()]
            
        pf_match = re.search(r"python_files\s*=\s*(.+)", config_text)
        if pf_match:
            python_files = [p.strip() for p in pf_match.group(1).split()]
...
def run_pipeline(issue_id):
    ...
    while attempt <= max_attempts and not pipeline_passed:
        ...
        print(f"\n[DEVOPS] Preparando intento {attempt}/{max_attempts}. Restaurando workspace a estado base...")
        ...
        if design is None: # Diseñar solo si no existe un plan previo
            ...
            # Aquí iría la validación del diseño contra el contrato canónico que propusiste.
        # En reintentos, no se replanifica, se reusa el 'design' validado.
        ...
        for act in code_actions + test_actions:
            ...
            # Si el archivo ya se generó bien en un intento previo y no tiene feedback, no lo regeneramos.
            if path in generated_files and path not in feedback_dict:
                continue

            if act in code_actions: 
                code = agent_implement_code(act, design, generated_files, feedback_dict.get(path, ""))
            else: 
                production_code_context = "\n\n".join([f"# Archivo: {p}\n{c}" for p, c in generated_files.items() if not is_test_file(p, c)])
                code = agent_generate_tests(act, production_code_context, feedback_dict.get(path, ""))
        ...
        if not syntax_passed:
            ...
            # No se replanifica, solo se reintenta la generación del archivo fallido
            attempt += 1
            continue
        # -------------------------------------------------------------
        ...
        if not success_qa:
            ...
            # No se replanifica, solo se reintenta la generación de los tests fallidos
            attempt += 1
            continue
